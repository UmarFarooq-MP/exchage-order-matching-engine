...
class ComparablePrice {
  ...
  public:
    ...
    ComparablePrice(bool buySide, Price price) : price_(price), buySide_(buySide) {}
    ...
    bool operator>(Price rhs) const {
        return price_ != MARKET_ORDER_PRICE &&
               ((rhs == MARKET_ORDER_PRICE) || (buySide_ ? (rhs > price_) : (price_ > rhs)));
    }
...
inline bool operator<(Price price, const ComparablePrice& key) {
    return key > price;
}
