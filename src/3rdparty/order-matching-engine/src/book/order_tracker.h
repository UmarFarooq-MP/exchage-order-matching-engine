...
OrderTracker<OrderPtr>::OrderTracker(const OrderPtr& order, OrderConditions conditions)
    : order_(order), open_qty_(order->order_qty()), reserved_(0), conditions_(conditions) {
...
template <class OrderPtr> Quantity OrderTracker<OrderPtr>::reserve(int64_t reserved) {
    reserved_ += reserved;
    return open_qty_ - reserved_;
}

template <class OrderPtr> void OrderTracker<OrderPtr>::change_qty(int64_t delta) {
    if ((delta < 0 && (int) open_qty_ < std::abs(delta))) {
        throw std::runtime_error("Replace size reduction larger than open quantity");
    }
    open_qty_ += delta;
}

template <class OrderPtr> void OrderTracker<OrderPtr>::fill(Quantity qty) {
    if (qty > open_qty_) {
...
template <class OrderPtr> bool OrderTracker<OrderPtr>::filled() const {
    return open_qty_ == 0;
}

template <class OrderPtr> Quantity OrderTracker<OrderPtr>::filled_qty() const {
...
template <class OrderPtr> Quantity OrderTracker<OrderPtr>::open_qty() const {
    return open_qty_ - reserved_;
}

template <class OrderPtr> const OrderPtr& OrderTracker<OrderPtr>::ptr() const {
    return order_;
}

template <class OrderPtr> OrderPtr& OrderTracker<OrderPtr>::ptr() {
    return order_;
}

template <class OrderPtr> bool OrderTracker<OrderPtr>::all_or_none() const {
    return bool(conditions_ & oc_all_or_none);
}

template <class OrderPtr> bool OrderTracker<OrderPtr>::immediate_or_cancel() const {
