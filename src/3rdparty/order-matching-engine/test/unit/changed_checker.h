...
template <int SIZE = 5> class ChangedChecker {
  public:
    typedef Depth<SIZE> SizedDepth;
    ChangedChecker(SizedDepth& depth) : depth_(depth) {
        reset();
    }
    ...
    bool verify_bbo_stamps(ChangeId bid_stamp, ChangeId ask_stamp) {
        ...
        if (depth_.bids()->last_change() != bid_stamp) {
            std::cout << "best bid change " << depth_.bids()->last_change() << std::endl;
            matched = false;
        }
        if (depth_.asks()->last_change() != ask_stamp) {
            std::cout << "best ask change " << depth_.asks()->last_change() << std::endl;
            matched = false;
        }
        return matched;
    }
  ...
  private:
    ...
    bool verify_side_stamps(
        const DepthLevel* start, ChangeId l0, ChangeId l1, ChangeId l2, ChangeId l3, ChangeId l4) {
        bool matched = true;
    ...
    bool verify_level(const DepthLevel* levels, size_t index, bool expected) {
        ...
        if (levels[index].changed_since(last_change_) != expected) {
            matched = false;
            if (expected) {
                std::cout << "expected change level[" << index << "] " << levels[index].price()
                          << std::endl;
            } else {
                std::cout << "unexpected change level[" << index << "] " << levels[index].price()
                          << std::endl;
            }
        }
        return matched;
