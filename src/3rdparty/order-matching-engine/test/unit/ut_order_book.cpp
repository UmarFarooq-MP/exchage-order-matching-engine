...
BOOST_AUTO_TEST_CASE(TestBidsMultimapSortCorrect) {
    ...
    bids.insert(
        std::make_pair(book::ComparablePrice(true, order0.price()), SimpleTracker(&order0)));
    bids.insert(
        std::make_pair(book::ComparablePrice(true, order1.price()), SimpleTracker(&order1)));
    bids.insert(
        std::make_pair(book::ComparablePrice(true, order2.price()), SimpleTracker(&order2)));
    bids.insert(
        std::make_pair(book::ComparablePrice(true, order3.price()), SimpleTracker(&order3)));
    bids.insert(
        std::make_pair(book::ComparablePrice(true, order4.price()), SimpleTracker(&order4)));

    // Should access in price order
    SimpleOrder* expected_order[] = {&order3, &order1, &order0, &order4, &order2};
    ...
    BOOST_CHECK(
        (bids.upper_bound(book::ComparablePrice(true, 1245)))->second.ptr()->price() == 1240);
    BOOST_CHECK(
        (bids.lower_bound(book::ComparablePrice(true, 1245)))->second.ptr()->price() == 1245);
}
...
BOOST_AUTO_TEST_CASE(TestAsksMultimapSortCorrect) {
    ...
    // Insert out of price order
    asks.insert(
        std::make_pair(book::ComparablePrice(false, order0.price()), SimpleTracker(&order0)));
    asks.insert(
        std::make_pair(book::ComparablePrice(false, order1.price()), SimpleTracker(&order1)));
    asks.insert(
        std::make_pair(book::ComparablePrice(false, order2.price()), SimpleTracker(&order2)));
    asks.insert(
        std::make_pair(book::ComparablePrice(false, MARKET_ORDER_PRICE), SimpleTracker(&order3)));
    asks.insert(
        std::make_pair(book::ComparablePrice(false, order4.price()), SimpleTracker(&order4)));
    asks.insert(
        std::make_pair(book::ComparablePrice(false, order5.price()), SimpleTracker(&order5)));

    // Should access in price order
    SimpleOrder* expected_order[] = {&order3, &order2, &order1, &order4, &order0, &order5};
    ...
    BOOST_CHECK(
        (asks.upper_bound(book::ComparablePrice(false, 3235)))->second.ptr()->price() == 3245);
    BOOST_CHECK(
        (asks.lower_bound(book::ComparablePrice(false, 3235)))->second.ptr()->price() == 3235);
}
...
BOOST_AUTO_TEST_CASE(TestReplaceSizeDecreaseCancel) {
    ...
    // Replace size - cancel
    BOOST_CHECK(replace_and_verify(order_book, &ask0, -175, PRICE_UNCHANGED, simple::os_cancelled));
    ...
    BOOST_CHECK(replace_and_verify(order_book, &bid1, -100, PRICE_UNCHANGED, simple::os_accepted));
    ...
    BOOST_CHECK(replace_and_verify(order_book, &bid1, -200, PRICE_UNCHANGED, simple::os_cancelled));
...
BOOST_AUTO_TEST_CASE(TestReplaceBidMatch) {
    ...
    // Match - complete
    {
        ...
        BOOST_CHECK(
            replace_and_verify(order_book, &bid1, SIZE_UNCHANGED, 1253, simple::os_complete, 140));
    }
...
BOOST_AUTO_TEST_CASE(TestReplaceAskMatch) {
    ...
    // Match - complete
    {
        ...
        BOOST_CHECK(
            replace_and_verify(order_book, &ask0, SIZE_UNCHANGED, 1251, simple::os_accepted, 140));
    }
