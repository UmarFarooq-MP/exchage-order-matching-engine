...
template <class OrderBook, class OrderPtr>
bool cancel_and_verify(
    OrderBook& order_book, const OrderPtr& order, simple::OrderState expected_state) {
    order_book.cancel(order);
    return expected_state == order->state();
}

template <class OrderBook, class OrderPtr>
bool replace_and_verify(
    OrderBook& order_book,
    const OrderPtr& order,
    int32_t size_change,
    ...
    Price expected_price = (new_price == PRICE_UNCHANGED) ? order->price() : new_price;
...
template <class OrderPtr> class FillCheck {
  public:
    FillCheck(OrderPtr order, Quantity filled_qty, Cost filled_cost, OrderConditions conditions = 0)
        : order_(order), expected_filled_qty_(order->filled_qty() + filled_qty),
          expected_open_qty_(order->order_qty() - expected_filled_qty_),
          expected_filled_cost_(order->filled_cost() + (filled_cost)), conditions_(conditions) {}
  ...
  private:
    ...
    void verify_filled() {
        if (expected_filled_qty_ != order_->filled_qty()) {
            std::cout << "filled_qty " << order_->filled_qty() << " expected "
                      << expected_filled_qty_ << std::endl;
            throw std::runtime_error("Unexpected filled quantity");
        }
        if (expected_open_qty_ != order_->open_qty()) {
            std::cout << "open_qty " << order_->open_qty() << " expected " << expected_open_qty_
                      << std::endl;
            throw std::runtime_error("Unexpected open quantity");
        }
        if (expected_filled_cost_ != order_->filled_cost()) {
            std::cout << "filled_cost " << order_->filled_cost() << " expected "
                      << expected_filled_cost_ << std::endl;
        ...
        if (order_->state() != simple::os_complete && !expected_open_qty_) {
            std::cout << "state " << order_->state() << " expected " << simple::os_complete
                      << std::endl;
            ...
            if (order_->state() != simple::os_accepted && !IOC) {
                std::cout << "state " << order_->state() << " expected " << simple::os_accepted
                          << std::endl;
                throw std::runtime_error("Unexpected state with open quantity");
            }
            if (order_->state() != simple::os_cancelled && IOC) {
                std::cout << "state " << order_->state() << " expected " << simple::os_cancelled
                          << std::endl;
