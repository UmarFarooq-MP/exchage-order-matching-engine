...
BOOST_AUTO_TEST_CASE(TestStopMarketOrdersTradeWhenStopPriceReached) {
    ...
    // Scope for fill checks
    {
        ...
        // Trade at 56 which should trigger order2 which should trade with order 1 at order 1's
        // price
        BOOST_CHECK(
            add_and_verify(book, &order4, expectNoMatch, expectNoComplete, book::oc_all_or_none));
        BOOST_CHECK(
            add_and_verify(book, &order5, expectMatch, expectComplete, book::oc_all_or_none));
    }
    ...
    // Scope for fill checks
    {
        ...
        // Trade at 54 which should trigger order3 which should trade with order 0 at order 0's
        // price
        BOOST_CHECK(
            add_and_verify(book, &order6, expectNoMatch, expectNoComplete, book::oc_all_or_none));
        BOOST_CHECK(
            add_and_verify(book, &order7, expectMatch, expectComplete, book::oc_all_or_none));
    }
