...
BOOST_AUTO_TEST_CASE(TestTwoAonBidOneAonAsk) {
    ...
    // Prime the order book: No Matches
    BOOST_CHECK(add_and_verify(
        order_book, &bid1, expectNoMatch, expectNoComplete, AON)); // AON)); //noConditions
    BOOST_CHECK(add_and_verify(order_book, &bid2, expectNoMatch, expectNoComplete, AON));
...
BOOST_AUTO_TEST_CASE(TestOneBidTwoAonAsk) {
    ...
    // Add matching order
    {
        ...
        SimpleFillCheck fc3(&ask2, qty2, qty2 * prc1);
        BOOST_CHECK(
            add_and_verify(order_book, &ask1, expectNoMatch, expectNoComplete, noConditions));
        BOOST_CHECK(add_and_verify(order_book, &ask2, expectMatch, expectComplete, noConditions));
    }
...
#else
    // The current match algorithm tries to match one order from one side of the market to "N"
    // orders from the other side.   This test won't pass because it requires two orders from each
    // side. I'm leaving the test here as a challenge to future developers who want to improve the
    // matching algorithm (good luck)
...
BOOST_AUTO_TEST_CASE(TestReplaceAonBidPriceMatch) {
    ...
    // Match - complete
    {
        ...
        BOOST_CHECK(
            replace_and_verify(order_book, &bid1, qtyNone, prc2, simple::os_complete, qty2));
    }
