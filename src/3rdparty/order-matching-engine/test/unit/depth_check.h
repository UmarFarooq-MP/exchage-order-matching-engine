...
template <typename SimpleOrderBook> class DepthCheck {
    typedef typename SimpleOrderBook::DepthTracker SimpleDepth;

  public:
    DepthCheck(const SimpleDepth& depth) : depth_(depth) {
        reset();
    }

    static bool
    verify_depth(const DepthLevel& level, const Price& price, uint32_t count, const Quantity& qty) {
        bool matched = true;
