...
void build_histogram(timespec* timestamps, int count) {
    ...
    for (timespec* timestamp = timestamps; (timestamp - timestamps) <= count; ++timestamp) {
        if (prev) {
            timespec elapsed;
...
int run_test(TypedOrderBook& order_book, TypedOrder** orders, timespec* timestamps) {
    int count = 0;
...
template <class TypedOrderBook> bool build_and_run_test(uint32_t num_to_try, bool dry_run = false) {
